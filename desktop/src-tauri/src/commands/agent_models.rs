...
pub async fn discover_agent_models(
    ...
    if let Some(models) =
        discover_bedrock_models(&state.http_client, &effective_provider, &merged_env, None).await?
    {
        return Ok(models);
    }
...
#[cfg(test)]
use databricks::is_databricks_provider;

#[path = "agent_models_bedrock.rs"]
mod bedrock;
use bedrock::discover_bedrock_models;
